  ...
  useEffect(() => {
    ...
    if (badges.length) {
      anims.push(gsap.from(badges, {
        scrollTrigger: { trigger: sectionRef.current, start: "top 80%", toggleActions: "play none none reverse" },
        immediateRender: false,
        opacity: 0, scale: 0.8, duration: 0.3, stagger: 0.02, ease: "back.out(1.7)",
      }));
    }
    ...
    <section ref={sectionRef} id="skills" className="min-h-screen flex items-center justify-center px-4 sm:px-6 py-16 sm:py-24">
      <div className="max-w-4xl w-full">
        <p className="text-terminal-green/50 font-mono text-[10px] sm:text-xs mb-2">
          luca@portfolio:~$ cat package.json | jq '.skills'
        </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2.5">
                    {skills.filter((s) => s.category === cat).map((skill) => {
                      ...
                      return (
                        <span
                          key={skill.name}
                          ref={(el) => { badgeRefs.current[bi] = el; }}
                          className={`font-mono text-[10px] sm:text-xs md:text-sm px-2 py-1 sm:px-3 sm:py-1.5 rounded-sm
                            bg-site-bg border ${config.border} ${config.color} ${config.glow}
